...
Actions.prototype.deploy = function() {
  ...
  buildApp(appPath, meteorBinary, buildLocation, {
    buildArgs: this.config.buildArgs,
    serverUrl: this.config.env.ROOT_URL
  }, function(err) {
    if(err) {
      process.exit(1);
    } else {
...
Actions.init = function() {
  var destMupJson = path.resolve('uup.json');
  ...
  var exampleMupJson = path.resolve(__dirname, '../example/uup.json');
