...
exports.remove = function(config) {
  ...
  if (config.nginx) {
    ...
    nginx_config_file = nginx_port + '_' + config.appName;
  }

  taskList.executeScript('Remove', {
    script: path.resolve(SCRIPT_DIR, 'remove-app.sh'),
    vars: {
...
exports.setup = function(config) {
  ...
  taskList.copy('Configuring upstart', {
    ...
    vars: {
      appName: config.appName,
      foreverCommand: config.foreverCommand || 'node'
    }
  });
