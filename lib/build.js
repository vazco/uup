...
function buildApp(appPath, meteorBinary, buildLocaltion, extraOptions, callback) {
  buildMeteorApp(appPath, meteorBinary, buildLocaltion, extraOptions, function(code) {
    if(code == 0) {
...
function buildMeteorApp(appPath, meteorBinary, buildLocaltion, extraOptions, callback) {
  var executable = meteorBinary;

  extraOptions = _(extraOptions || {}).defaults({
    buildArgs: '',
  ...
  var args = [
    ...
    "--server", extraOptions.serverUrl
  ];

  if (extraOptions.buildArgs) {
    args = args.concat(extraOptions.buildArgs.split(' '));
  }
  ...
  console.log('\nBuild command: ' + executable + ' ' + args.join(' ') + '\n');
